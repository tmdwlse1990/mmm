...
#define APPLY_CARDFIX_RE(damage, fix) { (damage) = (damage) - (int64)(((damage) * (100 - max(0, 100+(fix)))) / 100); }
				...
				sum_val += sd->indexed_bonus.magic_addsize[tstatus->size] + sd->indexed_bonus.magic_addsize[SZ_ALL];
				...
				if( !nk[NK_IGNOREELEMENT] ) { // Affected by Element modifier bonuses
					...
					sum_val += sd->indexed_bonus.magic_addele[tstatus->def_ele] + sd->indexed_bonus.magic_addele[ELE_ALL] +
			...
			if( sd && !nk[NK_IGNOREATKCARD] ) {
				if( !nk[NK_IGNOREELEMENT] ) {
					...
					sum_val += sd->indexed_bonus.magic_atk_ele[rh_ele] + sd->indexed_bonus.magic_atk_ele[ELE_ALL];
				}
				...
				sum_val += sd->indexed_bonus.magic_addrace[tstatus->race] + sd->indexed_bonus.magic_addrace[RC_ALL] + sd->indexed_bonus.magic_addclass[tstatus->class_] + sd->indexed_bonus.magic_addclass[CLASS_ALL];
				APPLY_CARDFIX_RE( damage , sum_val );
...
// Pre-renewal / old renewal behaviour
			...
			if( tsd && !nk[NK_IGNOREDEFCARD] ) {
...
#endif
					ele_fix = cap_value(ele_fix, -10000, 70);
					cardfix = cardfix * (100 - ele_fix) / 100;
				}
				int32 size_fix, magic_size_fix = 0;

				size_fix = tsd->indexed_bonus.subsize[tstatus->size] + tsd->indexed_bonus.subsize[SZ_ALL];
				magic_size_fix = tsd->indexed_bonus.magic_subsize[tstatus->size] + tsd->indexed_bonus.magic_subsize[SZ_ALL];

				size_fix = cap_value(size_fix, -10000, 70);
				magic_size_fix = cap_value(magic_size_fix, -10000, 70);

				cardfix = cardfix * (100 - size_fix) / 100;
				cardfix = cardfix * (100 - magic_size_fix) / 100;
				...
				race_fix = cap_value(race_fix, -10000, 70);

				cardfix = cardfix * (100 - race_fix) / 100;
				int32 class_fix = cap_value(tsd->indexed_bonus.subclass[tstatus->class_] + tsd->indexed_bonus.subclass[CLASS_ALL],-10000,70);

				cardfix = cardfix * (100 - class_fix) / 100;
			...
			if( sd && !nk[NK_IGNOREATKCARD] && (left&2) ) {
				...
				if( sd->state.arrow_atk ) { // Ranged attack
					//cardfix = cardfix * (100 + sd->right_weapon.addrace[tstatus->race] + sd->indexed_bonus.arrow_addrace[tstatus->race] +
						...
						sum_atk_val += sd->right_weapon.addrace[tstatus->race] + sd->indexed_bonus.arrow_addrace[tstatus->race] + sd->right_weapon.addrace[RC_ALL] + sd->indexed_bonus.arrow_addrace[RC_ALL];
					if( !nk[NK_IGNOREELEMENT] ) { // Affected by Element modifier bonuses
					...
					sum_atk_val += sd->right_weapon.addsize[tstatus->size] + sd->indexed_bonus.arrow_addsize[tstatus->size] +
					...
					//	sd->right_weapon.addclass[CLASS_ALL] + sd->indexed_bonus.arrow_addclass[CLASS_ALL]) / 100;
						sum_atk_val += race_fix + sd->right_weapon.addclass[tstatus->class_] + sd->indexed_bonus.arrow_addclass[tstatus->class_] + sd->right_weapon.addclass[CLASS_ALL] + sd->indexed_bonus.arrow_addclass[CLASS_ALL];
				} else { // Melee attack
					int32 skill = 0;
					...
					if( !battle_config.left_cardfix_to_right ) {
						...
						if( left&1 ) { // Left-handed weapon
							...
							if( !nk[NK_IGNOREELEMENT] ) { // Affected by Element modifier bonuses
								...
								sum_atk_val_ += ele_fix_lh;
							}
							//cardfix_ = cardfix_ * (100 + sd->left_weapon.addsize[tstatus->size] + sd->left_weapon.addsize[SZ_ALL]) / 100;
							sum_atk_val_ += sd->left_weapon.addsize[tstatus->size] + sd->left_weapon.addsize[SZ_ALL];
							for (const auto &raceit : t_race2)
							...
							sum_atk_val_ += sd->left_weapon.addrace2[raceit] + sd->left_weapon.addclass[tstatus->class_] + sd->left_weapon.addclass[CLASS_ALL];
						}
					}
...
#endif
				//cardfix = cardfix * (100 + ele_fix) / 100;
				cardfix = cardfix * (100 + sum_atk_val) / 100;
				cardfix_ = cardfix_ * (100 + sum_atk_val_) / 100;
			...
			// Affected by target DEF bonuses
				...
				if( !nk[NK_IGNOREELEMENT] ) { // Affected by Element modifier bonuses
					...
					if( left&1 && lh_ele != rh_ele ) {
						...
						ele_fix_lh = cap_value(ele_fix_lh, -10000, 70);
						cardfix = cardfix * (100 - ele_fix_lh) / 100;
					}
					int32 ele_def_fix = cap_value(tsd->indexed_bonus.subdefele[s_defele] + tsd->indexed_bonus.subdefele[ELE_ALL], -10000, 70);
					cardfix = cardfix * (100 - ele_def_fix) / 100;
				}

				int32 race_fix = 0;

				int32 size_fix = cap_value(tsd->indexed_bonus.subsize[tstatus->size] + tsd->indexed_bonus.subsize[SZ_ALL],-10000,70);
				int32 weapon_size_fix = cap_value(tsd->indexed_bonus.weapon_subsize[tstatus->size] + tsd->indexed_bonus.weapon_subsize[SZ_ALL], -10000, 70);

				cardfix = cardfix * (100 - size_fix) / 100;
				cardfix = cardfix * (100 - weapon_size_fix) / 100;
				for (const auto &raceit : s_race2)
					race_fix += tsd->indexed_bonus.subrace2[raceit];
				cardfix = cardfix * (100 - race_fix) / 100;
				race_fix = tsd->indexed_bonus.subrace[sstatus->race] + tsd->indexed_bonus.subrace[RC_ALL];
				...
				if( flag&BF_SHORT )
					cardfix = cardfix * (100 - min(70,tsd->bonus.near_attack_def_rate)) / 100;
				else if (!nk[NK_IGNORELONGCARD])	// BF_LONG (there's no other choice)
					cardfix = cardfix * (100 - min(70,tsd->bonus.long_attack_def_rate)) / 100;
...
int64 battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int64 damage,uint16 skill_id,uint16 skill_lv)
{
	...
	status_change* scc = status_get_sc(src);
	if (scc != nullptr && !scc->empty()) {
			int32 f_dm = 0;
			
			if (scc->getSCE(SC_MEMBER5)) f_dm = 1;
			if (scc->getSCE(SC_MEMBER6)) f_dm = 2;
			if (scc->getSCE(SC_MEMBER7)) f_dm = 3;
			if (scc->getSCE(SC_MEMBER8)) f_dm = 4;
...
static int32 battle_skill_damage(struct block_list *src, struct block_list *target, uint16 skill_id) {
	...
	rates = battle_skill_damage_skill(src, target, skill_id) + battle_skill_damage_map(src, target, skill_id) + get_bonus_skillrate(src, skill_id) - get_bonus_skillratedef(target,skill_id);
	return rates;
}
...
static bool is_attack_critical(struct Damage* wd, struct block_list *src, struct block_list *target, int32 skill_id, int32 skill_lv, bool first_call)
{
	...
	*/
	...
	if( sstatus->cri )
	{
		...
		if(sc && sc->getSCE(SC_TEMPERANCECARD))
			return true;
...
static void battle_calc_skill_base_damage(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch (skill_id) {	//Calc base damage according to skill
		case PA_SACRIFICE:
			wd->damage = sstatus->max_hp* 27/100;
			wd->damage2 = 0;
#ifdef RENEWAL
		...
		case RK_DRAGONBREATH_WATER:
			{
				...
				*/
				int32 damagevalue = sstatus->hp * skill_lv / 10;
...
static int32 battle_calc_attack_skill_ratio(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch(skill_id) {
...
#ifdef RENEWAL
		case CR_ACIDDEMONSTRATION:
			skillratio += -100 + 100 * skill_lv + sstatus->int_ + tstatus->vit; // !TODO: Confirm status bonus
		...
		case ASC_BREAKER:
#ifdef RENEWAL
			skillratio += -100 + 200 * skill_lv + sstatus->str + (sstatus->int_ * 6 ); // !TODO: Confirm stat modifier
			RE_LVL_DMOD(100);
#else
		...
		case PA_SHIELDCHAIN:
			...
			if( sd != nullptr ){
				...
				if( index >= 0 && sd->inventory_data[index] != nullptr && sd->inventory_data[index]->type == IT_ARMOR ){
					...
					skillratio += 20 * sd->inventory.u.items_inventory[index].refine * (sd->inventory.u.items_inventory[index].enchantgrade + 1);
				}
		...
		*/
		...
		case GC_COUNTERSLASH:
			...
			skillratio += -100 + 150 + 100 * skill_lv;
			RE_LVL_DMOD(120);
		...
		case SC_TRIANGLESHOT: 
			skillratio += -100 + 150 * skill_lv + 5 * sstatus->agi;
			RE_LVL_DMOD(100);
			break;
		case SC_FEINTBOMB:
			...
			break;
		case LG_CANNONSPEAR:
			skillratio += -100 + skill_lv * ( 200 + (sstatus->str * 6) );
			...
			default: {
				...
				switch(skill_id) {
					...
					case SO_EARTHGRAVE:
						...
						skillratio += -100 + (6 * sstatus->int_) + (100 * (pc_checkskill(sd, SA_SEISMICWEAPON) + skill_lv));
						RE_LVL_DMOD(100);
...
struct Damage battle_calc_misc_attack(struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv,int32 mflag)
{
	...
	// P.ATK Apply
	if (sd != nullptr) {
		md.damage = (int64)floor((float)(md.damage * (100 + sstatus->patk) / 100));
	}
...
void battle_adjust_conf()
{
	...
	battle_config.aspd_unlock_cap = (AMOTION_ZERO_ASPD - battle_config.aspd_unlock_cap * AMOTION_INTERVAL) * AMOTION_DIVIDER_PC;
