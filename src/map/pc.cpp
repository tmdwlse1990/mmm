...
void pc_bonus(map_session_data *sd,int32 type,int32 val)
{
	...
	switch(type){
		...
		case SP_NORMAL_RATIO:
			...
			break;
		case SP_HIT_PHYSICAL_DAMAGE_RATE:
			if (sd->state.lr_flag != LR_FLAG_ARROW)
				sd->bonus.hit_physical_damage_rate += val;
			break;
		case SP_HIT_MAGICAL_DAMAGE_RATE:
			if (sd->state.lr_flag != LR_FLAG_ARROW)
				sd->bonus.hit_magical_damage_rate += val;
			break;
		case SP_CRIT_ATK_DMG:
			if (sd->state.lr_flag != LR_FLAG_ARROW)
				sd->bonus.crit_atk_dmg += val;
			break;
		case SP_CRIT_MATK_DMG:
			if (sd->state.lr_flag != LR_FLAG_ARROW)
...
int64 pc_readparam(map_session_data* sd,int64 type)
{
	...
	switch(type) {
		...
		case SP_HIT_PHYSICAL_DAMAGE_RATE:     val = sd->bonus.hit_physical_damage_rate; break;
...
int32 pc_itemheal(map_session_data *sd, t_itemid itemid, int32 hp, int32 sp)
{
	...
	if (hp) {
		bonus = 100 + (sd->battle_status.vit * 10) + pc_checkskill(sd, SM_RECOVERY) * 10 + pc_checkskill(sd, AM_LEARNINGPOTION) * 5;
