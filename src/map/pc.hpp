...
public:
	...
	struct s_state {
		uint32 autoattack : 1; //Marks active player (not active is logging in/out, or changing map servers)
	...
	struct s_indexed_bonus {
		...
		int32 skill_rate[SKILLRATE_TYPE_ALL];
		int32 skill_rate_def[SKILLRATE_TYPE_ALL];
	} indexed_bonus;
